...
do
	...
	do
		...
		do
			echo "rkeys = $rkeys; nkeys = $nkeys; nthrds = $nthrds"
			#stdbuf -o 0 perflock numactl --cpunodebind=0 go run ./benchmark/micro-hybrid/ -rkeys $rkeys -nkeys $nkeys -nthrds $nthrds -exp | tee -a $fpath
			stdbuf -o 0 perflock numactl --physcpubind=+0-$nthrds go run ./benchmark/micro-hybrid/ -rkeys $rkeys -nkeys $nkeys -nthrds $nthrds -exp | tee -a $fpath
		done
	done
done
